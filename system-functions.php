...
require "config.php"; // Require to have DB connection info
	...
	function load_theme(){ //Load the theme (called from "index.php")
		...
		track_hits(); //Add one hit to the hit counter
	} //end function
...
<?php
	...
	function track_hits(){
		mysql_query("UPDATE ".tableprefix()."system SET `value` = `value` +1 WHERE `function` = 'hits'");
	} //end function
	function echohits(){
		//Use this to show off how many hits you've had
		$sqlquery = mysql_query("SELECT * FROM ".tableprefix()."system WHERE `function` = 'hits'");
